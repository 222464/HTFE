	...
	int2 layerSize, int inhibitionRadius, float localActivity, float minDerivative)
{
	...
	float newDerivative = thisActivation * (1.0f - thisActivation) * (numHigher < localActivity ? 1.0f : minDerivative);

	write_imagef(hiddenStates, hiddenPosition, (float4)(newState, newDerivative, 0.0f, 0.0f));
}
